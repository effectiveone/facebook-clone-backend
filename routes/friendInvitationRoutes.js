...
router.get('/pending', authUser, async (req, res) => {
  try {
    ...
    logger.info(
      `Pobrano ${pendingInvitations.length} oczekujących zaproszeń dla użytkownika ${req.user.id}`,
    );
...
router.get('/sent', authUser, async (req, res) => {
  try {
    ...
    logger.info(
      `Pobrano ${sentInvitations.length} wysłanych zaproszeń od użytkownika ${req.user.id}`,
    );
